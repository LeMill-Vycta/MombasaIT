        ...
        <div class="col-lg-12">
            <div class="card"> 
                <div class="card-header">
                  <h4 class="card-title">Appointments list:</h4></br> 
                  <span>
                    <p class="lead">Total: {{$bookings->count()}}</p>
                  ...
                  <table class="table header-border table-hover verticle-middle">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td class="font4">{{$booking->user->phone_number ?? null ?: 'N/A'}}</td>
                          <td class="font4">{{$booking->user->email ?? null ?: 'N/A'}}</td>
                          <td class="font4">{{ucfirst($booking->service->name ?? null ?: 'N/A')}}</td>
                          <td class="font4">{{$booking->time}}</td>
                          <td class="font4">{{$booking->date}}</td>
                          <td>
                          @if($booking->status==0)
                              <a href="{{route('change.status',[$booking->id])}}"><button class="btn btn-sm btn-primary"> Pending</button></a>
                              @else 
                               <a href="{{route('change.status',[$booking->id])}}"><button class="btn btn-sm btn-success"> Approved</button></a>
                              @endif
                          </td>
                        </tr>
                        @empty
                        <td class="font4">There are no appointments today</td>
                        @endforelse 
